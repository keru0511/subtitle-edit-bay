...
`字幕を編集する` で専用の字幕編集画面へ切り替わり、動画を再生しながらテキスト・開始時刻・終了時刻・話者・字幕ごとのサイズ倍率を表形式で変更できます。実波形付きタイムラインでは字幕ブロックの移動と左右端のリサイズができ、ズーム、グリッド／字幕端スナップ、追加、分割、削除、元に戻す／やり直す、700ms後の自動保存に対応します。`ASSを更新` は動画を書き出さず、編集済みASSと画面内プレビューだけを更新します。
