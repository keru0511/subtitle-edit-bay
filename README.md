...
メイン画面の `次の操作` には `字幕を編集する` と `音量を調整する` が同じ階層で表示されます。`音量を調整する` で独立したミキサー画面へ切り替わり、動画内の各音声トラックと話者ごとの個別音声をチャンネルストリップとして横並びに表示します。再生ボタン、シークバー、波形付きシーケンスで素材を確認でき、縦型dBフェーダー、有効／無効、ミュート、ソロを操作できます。個別音声には文字起こし時の同期オフセットを適用して完成動画へAACで合成します。既定状態は従来どおり動画の先頭音声だけが有効です。
