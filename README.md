...
メイン画面は `素材`、`文字起こし`、`字幕編集`、`書き出し` の4工程を表示し、その時点で必要な操作だけを右側の `次の操作` に出します。処理デバイスや字幕サイズなどは `詳細設定` を開いた時だけ表示されます。動画・複数の話者音声・出力先を指定して文字起こしすると、raw transcriptとは別に `*.subtitle-project.json` が作成されます。このプロジェクトがユーザー編集の正本で、ASSと完成動画は何度でも作り直せます。
...
字幕編集画面の `音量ミキサー` では、動画内の各音声トラックと話者ごとの個別音声を独立したチャンネルとして、有効／無効、0〜200%の音量、ミュート、ソロを設定できます。個別音声には文字起こし時の同期オフセットを適用し、完成動画の書き出し時にAACへ合成します。既定状態は従来どおり動画の先頭音声だけが有効です。
