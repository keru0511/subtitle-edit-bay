...
WhisperXのraw認識結果が `「○○だよね」`、`「。○○なんだけど」` のように区切られた場合、字幕の確定前に `「○○だよね。」`、`「○○なんだけど」` へ補正します。対象は `、。！？!?，．,.` で始まる字幕です。複数の記号が連続する場合もまとめて、同じ文字起こし元の直前字幕へ移します。別音声ファイルや別音声トラックへは移しません。単一トラックを話者分離した境界では、先頭記号に発話タイミングがないため直前話者の字幕へ戻します。

この規則はWhisperXへのpromptではなく、すべての文字起こし経路が合流する `refine_segments` で適用します。`transcripts/*.json` のraw認識結果は変更せず、`*.craig.merged.json` と新しく作る字幕プロジェクトだけを補正します。日本語字幕の構造を守る既定規則であり、現在は設定で無効化しません。戻し先を安全に特定できない最初の字幕では、認識文字を勝手に削除しないため記号を保持します。
