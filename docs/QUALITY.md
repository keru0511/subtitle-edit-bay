...
これらの入力もobjectとして検証し、数値文字列や既存の丸め・既定値を維持します。単語データは辞書の配列、波形ピークは数値の反復可能な値として検証し、不正な構造・要素は`SubtitleProjectError`で拒否します。数値以外の波形ピークを暗黙に数値化しません。字幕・話者・波形の拡張データは保持し、音声ミックスの読み込みでは従来どおり未知フィールドを取り込みません。GUI側のレイアウト・組版処理はまだAny禁止の対象外です。
...
`subtitle_project.py`全体では、JSON読込値を`object`として受けてから検証し、保存・文字起こし取込・表示用データ・レイアウト行割当・波形生成までAnyを禁止します。未知の拡張フィールドと保存時の元辞書更新を維持します。品質ジョブでNumPy本体を導入せず型チェックするため、波形集計で実際に使うNumPy APIだけを`typings/numpy/`に定義し、対応する型付きテストを実ライブラリでも実行します。

`timeline_interchange.py`では、JSON読込値を`object`として受け、スキーマとプロジェクトの辞書構造を確認します。正規形式では未知フィールドと従来形式との競合検査を維持し、EDL出力では数値文字列、単一のソース辞書、表現できない機能の警告を維持します。対応する入出力テストもAny禁止の対象です。
