...
既存の品質チェック用スクリプトとOS境界4モジュールに加え、実行設定の読み込み・検証・データ境界、文字起こしコンテキスト・辞書モデルと対応するテストを対象にしています（計17ファイル）。OS境界はstrictと明示的な`Any`禁止を継続します。移行済みのデータ境界・モデル・対応テストの12ファイルでは、次の規則をすべて適用します。
...
2. 字幕プロジェクト・文字起こし・辞書などのデータモデルとJSON契約（今回は文字起こしコンテキスト・辞書を移行、字幕プロジェクトは後続）
