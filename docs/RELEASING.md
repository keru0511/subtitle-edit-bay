...
例は `v0.1.0`、`v0.1.1`、`v1.0.0` です。公開済みのタグを削除・付け替えしないでください。公開後に問題が見つかった場合は、修正してパッチ番号を上げた新しいタグを作成します。

## タグ作成前チェック

1. リリース対象を `main` へ反映し、リモートと同期します。
2. 作業ツリーに意図しない変更がないことを確認します。
3. 自動テストを実行します。
4. README、利用ガイド、設定ガイドに利用者向けの変更が反映されていることを確認します。
5. APIキー、トークン、入力素材、出力動画、個人設定が含まれていないことを確認します。
6. 可能なら、まっさらなWindows環境でインストールと起動を確認します。
...
4. SHA-256チェックサムを検証する
5. 同じタグのGitHub Releaseを作成し、リリースノートを生成する
6. `SubtitleEditBay-Setup.exe` と `SubtitleEditBay-Setup.exe.sha256` を添付する
...
3. Assetsに `SubtitleEditBay-Setup.exe` とチェックサムファイルが表示されることを確認します。
