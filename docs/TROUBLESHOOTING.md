...
- app-serverが起動できなくても、手動の字幕編集・保存・書き出しは利用できる

Codexチャットで `切断` または `送信失敗` が表示された場合は、応答中でないことを確認して `再接続` を押します。`未ログイン` に戻った場合は再度ログインしてください。モデルエラーが表示された場合はモデル一覧から別の利用可能モデルを選択します。保存済みモデルが提供終了になった場合は、App Serverの既定モデルへ自動で切り替わります。

認証URLや会話本文は通常ログへ出力されません。問い合わせ時は、認証画面のURL全体、token、APIキー、会話に含まれる機密情報をIssueへ貼り付けないでください。

診断をIssueへ貼り付ける場合は、GUIの `診断をコピー` または `ログをコピー` を使用してください。APIキー、token、authorization、字幕本文、不要なローカルパスはredactionされます。実アカウントでの再現が必要な場合は、通常CIへ認証情報を追加せず、`RUN_CODEX_INTEGRATION=1` をローカルで明示して実行します。
