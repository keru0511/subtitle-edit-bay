...
診断をIssueへ貼り付ける場合は、GUIの `エラーをコピー` を使用してください。APIキー、token、authorization、字幕本文、不要なローカルパスはredactionされます。実アカウントでの再現が必要な場合は、通常CIへ認証情報を追加せず、`RUN_CODEX_INTEGRATION=1` をローカルで明示して実行します。
