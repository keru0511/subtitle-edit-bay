...
| --- | --- |
| 素材一覧の配置・表示条件 | 常設化済み。字幕専用ソース選択への切り替えは未対応 |
...
| シーケンスタイムライン | 下部へ移動済み。横方向の映像・音声・BGM/SEトラック表示は未対応 |
| 結合設定・BGM/SE | 既存のクリップ単位の設定を維持。OP/ED、全体トランジション、独立BGM/SEは未対応 |
| 字幕絞り込み | クリップ別フィルターは未対応 |
| インスペクター | 設定とAIを同じタブ枠に収める変更は未対応 |
| 操作導線 | ヘッダーとの重複を整理済み。左レールのホーム・設定は未対応 |
...
1. 動画プロジェクトを開き、字幕・カット・音量を切り替える。どのモードでも左側に素材一覧が残ることを確認する。
2. 素材を追加し、「カット」→「動画結合・並べ替え」を開く。追加素材を配置済みクリップへドラッグし、直前に追加されることを確認する。
3. Undo・Redo、保存・再読み込みで挿入順序を確認する。
4. 1220×760、1520×940で素材一覧・プレビュー・下部編集領域が重ならず、設定とAIログインにアクセスできることを確認する。
