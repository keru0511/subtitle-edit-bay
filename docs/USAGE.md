...
メイン画面の `次の操作` にある `音量を調整する` から、字幕編集とは独立したミキサー画面を開きます。動画内の音声トラックと素材設定で追加した個別音声が、実機ミキサーのような縦型チャンネルストリップで横並びに表示されます。画面上部の再生ボタン、±5秒移動、再生位置へ追従するシークバー、INPUT ONの各チャンネルを表示する音量バー／波形付きシーケンスで出力音と再生位置を確認できます。再生位置はメイン画面と字幕編集画面へ引き継がれ、シーケンス上をクリックしても移動できます。INPUT ON、ミュート、ソロ、フェーダーの相対バランスは再生中にも即時反映され、各チャンネルのレベルメーターはデコードした音声ピークへ追従します。各チャンネルは−60〜+6dBのフェーダー、有効／無効、ミュート、ソロを持ちます。1つ以上をソロにすると、ソロ中の有効チャンネルだけを書き出します。`全チャンネルをリセット` では動画の既定トラックを100%、個別音声を無効へ戻します。

初回だけ、ミキサー画面に「プレビュー音声を準備中…」と表示されます。動画内の各トラックと個別音声をプレビュー専用MKAへバックグラウンドでキャッシュし、準備完了後に再生操作が有効になります。キャッシュは `%LOCALAPPDATA%\Subtitle Edit Bay\audio-preview` に保存され、元ファイルのパス・サイズ・更新日時・トラック番号が同じ間は再利用されます。

個別音声は文字起こし時に検出した動画との同期オフセットを、ライブプレビューと完成動画の両方へ適用します。複数チャンネル再生時はクリッピングを避けるためプレビュー全体を正規化し、フェーダー間の相対バランスを保ちます。完成動画では指定したゲインをそのまま適用し、音声をAACへ再エンコードします。無音部分カットを有効にした場合も同じミックス設定を使用します。
