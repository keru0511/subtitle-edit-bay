...
メイン画面の `次の操作` にある `音量を調整する` から、字幕編集とは独立したミキサー画面を開きます。動画内の音声トラックと素材設定で追加した個別音声が、実機ミキサーのような縦型チャンネルストリップで横並びに表示されます。画面上部の再生ボタン、±5秒移動、再生位置へ追従するシークバー、波形付きシーケンスで出力音と再生位置を確認できます。再生位置はメイン画面と字幕編集画面へ引き継がれ、シーケンス上をクリックしても移動できます。INPUT ON、ミュート、ソロ、フェーダーの相対バランスは再生中にも即時反映され、各チャンネルのレベルメーターはデコードした音声ピークへ追従します。各チャンネルは−60〜+6dBのフェーダー、有効／無効、ミュート、ソロを持ちます。1つ以上をソロにすると、ソロ中の有効チャンネルだけを書き出します。`全チャンネルをリセット` では動画の既定トラックを100%、個別音声を無効へ戻します。
