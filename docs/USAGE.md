...
メイン画面上部には次の4工程と、その直下のコンテキスト操作バーが表示されます。現在の工程に応じた主操作・副操作・停止操作は操作バーから選択します。

1. `素材`: 動画、話者音声、出力先を指定
2. `文字起こし`: 話者音声の同期、WhisperX文字起こし、実波形生成、編集プロジェクト作成までを実行
3. `字幕・カット・音量編集`: 字幕、通常動画のカット、音量を共通ワークスペースで編集
4. `書き出し`: 保存済みプロジェクトからASSを再生成し、動画へ焼き付け

`詳細設定` を押すと、処理デバイス、Whisperモデル、字幕サイズ、音量連動、動画・音声設定を開閉できます。基準文字サイズは既定の50pxを100%として10〜900%で指定します。通常は閉じたまま、工程とプレビューを確認できます。

`文字起こし辞書を設定` を押すと、メイン画面から独立した辞書画面へ切り替わります。ゲームタイトル、確認済みの固有名詞、補足メモ、辞書JSONを編集し、`保存` または `メインへ戻る` で `.gui/runtime_config.json` へ保存します。辞書画面は詳細設定や動画プレビューへ重なりません。

素材設定は右上の `素材設定` から開きます。動画と話者音声はファイル選択に加え、素材設定内のドロップ欄またはアプリ画面へ直接ドラッグ＆ドロップできます。動画は選択中の1件を置き換え、音声は既存の話者音声へ追加します。出力先フォルダは `選択` から指定します。

動画と出力先だけを指定して `空の編集プロジェクトを作成` を押すと、文字起こしなしで `<動画名>.subtitle-project.json` を作成できます。動画内音声と素材設定で選択済みの外部音声はプロジェクトへ登録され、字幕の `segments` は空配列のままです。`字幕を編集する` を開き、再生位置で `+ 字幕追加` を押せば手動字幕から作業を始められます。

空のプロジェクトでも `音量を調整する`、`ショート動画を作成`、`動画を書き出す` を利用できます。ショート動画は字幕セグメントと無関係に開始・終了時刻を指定でき、字幕0件の通常動画・ショート動画の書き出しではASS生成と字幕フィルターを省略して音声調整・エンコードだけを行います。

文字起こしはプロジェクト作成後の任意のタイミングで実行できます。既存字幕がある状態で `文字起こしを追加 / 更新` を押すと、`置き換え`、`追加・統合`、`キャンセル` を選択できます。`追加・統合` は既存の手動字幕を残したまま新しい結果を追加し、既存プロジェクトを確認なしに削除しません。

文字起こし済みの既存プロジェクトは従来どおり `プロジェクトを開く` から読み込めます。raw WhisperX JSONと自動生成した `craig.merged.json` は入力記録として残り、ユーザー編集はプロジェクトJSONだけへ保存されます。このため、編集後に `字幕を焼き付ける` を何度実行してもWhisperXは起動しません。

### 字幕エディター

話者ごとの字幕色は、素材画面の話者名横にある色、または編集画面上部の `話者ごとの字幕色` から変更できます。色は全字幕でその話者へ共通適用され、プレビューと動画書き出しの両方へ反映されます。選択内容は自動保存され、次回起動時も維持されます。

表では字幕ごとに次を変更できます。

- 字幕本文（複数行入力に対応し、`Enter` で手動改行）
- 開始時刻と終了時刻（秒、小数第3位まで）
- 話者
- Windowsにインストール済みのフォント（未指定なら既定フォント）
- 自動算出された文字サイズ倍率（50〜200%）

タイムラインは話者ごとの実音声波形と字幕ブロックを表示します。動画の再生中は、現在表示中の字幕を字幕一覧で自動選択し、再生位置が見えるよう一覧とタイムラインを自動スクロールします。ブロック本体をドラッグすると区間全体を移動でき、選択中ブロックの白い左右端をドラッグすると開始・終了時刻を変更できます。`表示倍率` で時間軸を拡大縮小し、`スナップ` で10ms〜1000msのグリッドを指定します。移動先が別字幕の開始・終了端に近い場合は、その端へもスナップします。`スナップ=0` はスナップ無効です。

`+ 字幕追加` は現在の再生位置へ字幕を追加し、`分割` は選択字幕を再生位置で分割、`削除` は削除します。`元に戻す` / `やり直す` は最大100操作を保持します。キーボードでは `Ctrl+Z`、`Ctrl+Y`、`Ctrl+S`、`Delete` を使用できます。

字幕本文の編集欄には、自動整形で決まった改行位置も表示されます。`Enter` で改行を入れ直すと、その位置を自動整形より優先してプレビューとASSへ反映します。字幕ごとの行数選択はなく、手動改行がない本文は文字幅と表示時間を基に自動整形されます。入力中の本文と改行はフォーカスを外す前からプレビューで確認できます。

編集は変更後700msでプロジェクトへバックグラウンド保存されます。保存中に続けて変更した場合は最新の編集まで自動的に追って保存されます。ヘッダーの `● 編集あり` は保存待ち、`✓ 保存済み` は保存済みです。`ASSを更新` は動画を書き出さず `<動画名>.edited.ass` を作り、画面内の字幕プレビューにも本文・改行・話者色・字幕単位のフォント・サイズ倍率を反映します。

### 通常動画のカット編集

左のモードレールで `カット` を選び、素材タイムラインをドラッグして `選択範囲をカット` を押します。開始・終了秒は右側で直接入力でき、既存カットの範囲変更、全体または一部の復元、全解除、元に戻す／やり直すにも対応しています。右側にはカット適用後の長さを表示します。

カットは元動画や元字幕を変更せず、除外範囲だけをプロジェクトへ保存します。同じ中央プレイヤーで再生すると除外範囲を飛ばし、字幕と音声も完成動画と同じ位置関係で確認できます。編集自体にWhisperX、CUDA、NVENCは不要です。保存形式、素材／出力時間の境界規則、自動無音カットとの組み合わせは[通常動画の非破壊カット](NON_DESTRUCTIVE_CUTS.md)を参照してください。

### 音量ミキサー

コンテキスト操作バーの `音量を調整する` から、字幕編集とは独立したミキサー画面を開きます。動画内の音声トラックと素材設定で追加した個別音声が、実機ミキサーのような縦型チャンネルストリップで横並びに表示されます。画面上部の再生ボタン、±5秒移動、再生位置へ追従するシークバー、INPUT ONの各チャンネルを表示する音量バー／波形付きシーケンスで出力音と再生位置を確認できます。再生位置はメイン画面と字幕編集画面へ引き継がれ、シーケンス上をクリックしても移動できます。INPUT ON、ミュート、ソロ、フェーダーは再生中にも即時反映され、各チャンネルのレベルメーターに加えてマスターメーターとリミッターのゲインリダクションを確認できます。各チャンネルは−60〜+6dBのフェーダー、有効／無効、ミュート、ソロを持ちます。1つ以上をソロにすると、ソロ中の有効チャンネルだけを書き出します。`全チャンネルをリセット` では動画の既定トラックを100%、個別音声を無効へ戻します。

初回だけ、ミキサー画面に「プレビュー音声を準備中…」と表示されます。動画内の各トラックと個別音声を48kHzステレオFLACのプレビュー専用MKAへバックグラウンドでキャッシュし、準備完了後に再生操作が有効になります。キャッシュは `%LOCALAPPDATA%\Subtitle Edit Bay\audio-preview` に保存され、元ファイルのパス・サイズ・更新日時・トラック番号が同じ間は再利用されます。

個別音声は文字起こし時に検出した動画との同期オフセットを、ライブプレビューと完成動画の両方へ適用します。ライブプレビューは各トラックを48kHzステレオPCMとして同期し、フェーダー、ミュート、ソロを適用して1本のマスターバスへ加算します。フェーダーは100%を1.0、200%を2.0として扱い、INPUT ONの本数では既存チャンネルのゲインを変更しません。加算後は−1.5dBFSのリンクドステレオ・リミッターを通し、マスターメーターとゲインリダクションへ表示します。完成動画と無音部分カットにも同じフェーダーゲインとリミッターを適用し、音声をAACへ再エンコードします。
...
timeline             # 通常動画から除外する元素材基準のカット範囲
render_settings      # 最後に使った書き出し設定
~~~

プロジェクト内の時刻は常に元動画基準です。手動カットまたは `無音部分をカット` を有効にした場合だけ、レンダー時に字幕・動画・音声を同じkeep rangeで再配置します。これにより、カット設定を変えても編集済みの元タイムラインは失われません。通常動画の `timeline` とショート動画の `short_video` は独立しています。
...
既存の `*.subtitle-project.json` がある場合、CLIの文字起こしフェーズは手動編集を守るため上書きを拒否します。意図的に作り直す場合だけCLIへ `--overwrite-project` を追加してください。GUIではプロジェクト作成後も `文字起こしを追加 / 更新` を利用できますが、既存字幕の取り込み方法を選択する確認が必ず表示されます。
...
1. 文字起こし済みの `*.subtitle-project.json` をGUIで開く
2. 字幕セグメントが表示された状態で、コンテキスト操作バーから `ショート動画を作成` を押す
3. ショート画面でプレビューとクリップ一覧を確認する

字幕セグメントがないプロジェクトではショートモードを開けません。先に文字起こしを完了するか、既存の字幕プロジェクトを開いてください。

### クリップと画面設定

- クリップ一覧から字幕区間を追加し、上下ボタンで並べ替える
- 各クリップの開始・終了時刻は参照元セグメント内で指定する
- `cover` は画面全体を埋める代わりに左右または上下を中央クロップする
- `contain` は映像全体を残し、余白を背景色で埋める
- `blur` は拡大したぼかし背景の上に映像全体を配置する
- グローバルのfitと背景色は、クリップ側で個別指定しない場合に使われる
- 字幕スケールはショート出力の字幕全体の基準倍率で、字幕ごとの倍率・話者色・改行も反映される

### BGMとクロスフェード

設定欄のBGMで音声ファイルと次の値を指定します。

| 項目 | 意味 |
|---|---|
| `IN` | BGMファイルから読み始める位置（秒） |
| `OUT` | BGMファイルから読み終える位置（秒）。短いBGMは出力時間に合わせてループする |
| `START` | ショート動画の先頭からBGMを遅らせる時間（秒） |
| `volume` | BGMの音量。0は無音、1は基準音量 |

クリップ間のクロスフェードは、映像と元音声を同じ長さでつなぎます。短いクリップでは、クロスフェード時間を短くするか `cut` を選択してください。

### 書き出しと出力先

1. クリップ、fit、字幕スケール、BGM、トランジションを確認する
2. `ショート動画を書き出す` を押す
3. 完了後、元プロジェクトと同じ出力フォルダに `<プロジェクト名>.short.mp4` が作成される
4. 出力動画を再生し、字幕の位置とクリップ切替を確認する

出力は既定で1080x1920、30fps、H.264/AAC、MP4です。既存の完成動画は原子的に置き換えられ、失敗時は完成途中のファイルを正本として残しません。

### 対応形式と制約

入力はFFmpegが読み込める動画と音声を使用してください。書き出しには `ffmpeg` と `ffprobe` が必要です。FFmpeg 6以降を推奨し、長いfilter graphはバージョンに応じた一時スクリプト経由で実行します。

現在は、1つの字幕プロジェクト内の動画を参照する構成が対象です。複数動画ソース、候補区間の自動抽出、GUI上のサムネイル編集、SNS別テンプレートは未対応です。
