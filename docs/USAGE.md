...
編集は変更後700msでプロジェクトへバックグラウンド保存されます。保存中に続けて変更した場合は最新の編集まで自動的に追って保存されます。ヘッダーの `● 編集あり` は保存待ち、`✓ 保存済み` は保存済みです。`ASSを更新` は動画を書き出さず `<動画名>.edited.ass` を作り、画面内の字幕プレビューにも本文・話者色・字幕単位のフォント・サイズ倍率を反映します。
