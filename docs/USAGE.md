...
メイン画面の `次の操作` にある `音量を調整する` から、字幕編集とは独立したミキサー画面を開きます。動画内の音声トラックと素材設定で追加した個別音声が、実機ミキサーのような縦型チャンネルストリップで横並びに表示されます。画面上部の再生ボタン、±5秒移動、シークバー、波形付きシーケンスで再生位置を確認できます。再生位置はメイン画面と字幕編集画面へ引き継がれ、シーケンス上をクリックしても移動できます。各チャンネルは−60〜+6dBのフェーダー、有効／無効、ミュート、ソロを持ちます。1つ以上をソロにすると、ソロ中の有効チャンネルだけを書き出します。`全チャンネルをリセット` では動画の既定トラックを100%、個別音声を無効へ戻します。

個別音声は文字起こし時に検出した動画との同期オフセットを自動適用してから合成します。ミキサーを変更した完成動画の音声はAACへ再エンコードされます。ミキサー画面のフェーダーは完成動画の書き出し設定であり、リアルタイム音声モニターではありません。ミックス結果は動画の書き出し時に反映されます。無音部分カットを有効にした場合も同じミックス設定を使用します。
