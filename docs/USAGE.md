...
`+ 字幕追加` は現在の再生位置へ字幕を追加し、`分割` は選択字幕を再生位置で分割、`削除` は削除します。`元に戻す` / `やり直す` は最大100操作を保持します。キーボードでは `Ctrl+Z`、`Ctrl+Y`、`Ctrl+S`、`Delete` を使用できます。字幕本文の入力中は `Ctrl+Z` / `Ctrl+Y` / `Delete` を文字編集へ使用し、字幕全体の操作にはなりません。

字幕本文の編集欄には、自動整形で決まった改行位置も表示されます。出力プレビューが省略表示になる長文でも、編集欄には省略前の全文を自動改行付きで表示します。`Enter` で改行を入れ直すと、その位置を自動整形より優先してプレビューとASSへ反映します。手動改行で区切った各行が表示幅を超える場合は、その行の中だけを追加で自動折り返しします。字幕ごとの行数選択はなく、手動改行がない本文は文字幅と表示時間を基に自動整形されます。入力中の本文と改行はフォーカスを外す前からプレビューで確認できます。

編集は変更後700msでプロジェクトへバックグラウンド保存されます。保存中に続けて変更した場合は最新の編集まで自動的に追って保存されます。ヘッダーの `● 編集あり` は入力中の未確定変更または保存待ち、`✓ 保存済み` は保存済みです。`Ctrl+S`、`ASSを更新`、`字幕を焼き付ける`、`Escape`、`メインへ戻る`、アプリ終了では入力中の本文と改行を先に確定します。`ASSを更新` は動画を書き出さず `<動画名>.edited.ass` を作り、画面内の字幕プレビューにも本文・改行・話者色・字幕単位のフォント・サイズ倍率を反映します。
