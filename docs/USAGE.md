...
編集画面上部の `音量ミキサー` では、動画内の音声トラックと素材設定で追加した個別音声を1ファイルずつ操作できます。各チャンネルは有効／無効、0〜200%の音量、ミュート、ソロを持ちます。1つ以上をソロにすると、ソロ中の有効チャンネルだけを書き出します。`既定値に戻す` では動画の既定トラックを100%、個別音声を無効へ戻します。

個別音声は文字起こし時に検出した動画との同期オフセットを自動適用してから合成します。ミキサーを変更した完成動画の音声はAACへ再エンコードされます。編集画面のリアルタイム再生は元動画音声のままで、ミックス結果は `字幕を焼き付ける` で書き出した動画に反映されます。無音部分カットを有効にした場合も同じミックス設定を使用します。
