...
### 5.5 ショート専用カット＆シーケンス編集ドック

ショート動画の制作では、60秒の制限時間内に収めつつテンポの良いジャンプカットを行うため、9:16プレビュー直下に専用のカット＆シーケンス編集ドックを配置する。

1. **シーケンストラック (`#short-timeline-track`)**:
   - 採用クリップが尺に応じた比率で並び、クリック/シークで即座にプレビュー位置を移動。
   - クリップ両端のトリムハンドルによるIn/Out点調整。
2. **ショート専用カット操作ツールバー**:
   - `[ I ] 開始カット` / `[ O ] 終了カット`: 現在の再生ヘッド位置をクリップの開始点/終了点に更新。
   - `✂️ ここでカット(分割)`: 再生位置でクリップを前編・後編に二分割し、不要な発話や沈黙区間をカット除去。
   - ワンタッチ微調整: `-0.5s開始`, `+0.5s開始`, `-0.5s終了`, `+0.5s終了` によるクイックトリミング。
3. **60秒制限メーター**:
   - 各クリップの尺およびクロスフェード重なり（overlap）を考慮した総尺をリアルタイム集計。60秒超過時は警告表示。
4. **本編カット編集モードとの相互連携（ブリッジ機能）**:
   - 本編編集（カットモード）のインスペクター内に「📱 この範囲をショート動画に追加」ボタンを配置し、本編で選定したハイライト区間をショートのクリップとしてシームレスに転送・登録可能とする。
