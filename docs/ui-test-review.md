...
キー入力と実クリック／保存ショートカットを使う回帰テストを9件追加した。通常編集・拡大編集の保存と書き出し、ヘッダー保存、プレビュー更新、保存失敗後の再試行を対象とする。書き出しはプロセス開始時に保存済み本文を確認し、既存の実動画生成テストも併せて検証する。

再起動後の再読込テストの子プロセスは、保存検証と無関係な外部AI接続を無効にし、一時プロジェクト内の設定で起動するようにした。保存・書き出しの内部コード文字列を要求する検査は実操作テストに置き換えた。

修正後の `tests.test_gui_editor` と `tests.test_qml_static` は計224ケースすべて成功。Ruff、CIテスト分類検証、差分チェックも成功した。
