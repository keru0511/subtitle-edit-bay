...
追加監査では、Qt GUIの回帰テスト全体がLinuxでのみ実行されていたため、Windows CIでも処理中の本文・時刻の保持、一覧のスクロール、タイムラインの移動・両端の伸縮を実操作で確認する対象ケースを指定した。入力途中で検証上は未確定となる時刻も、処理終了後に続きを入力して保存できることを確認する。
