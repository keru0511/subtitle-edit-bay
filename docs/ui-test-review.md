...
キー入力と実クリック／保存ショートカットを使う回帰テストを13件追加した。通常編集・拡大編集の保存と書き出し、ヘッダー保存、プレビュー更新、保存失敗後の再試行を対象とする。書き出しはプロセス開始時に保存済み本文を確認し、既存の実動画生成テストも併せて検証する。
...
追加修正後の `tests.test_gui_editor`・`tests.test_qml_static`・`tests.test_production_integration` は計230ケースすべて成功。portableテストも1087ケースで失敗・エラーなし（Windows専用4ケースはスキップ）。Ruff、CIテスト分類検証、差分チェックも成功した。
