...
補助操作として「文字起こしから始める」「素材設定」「文字起こし辞書」を配置します。「素材設定」では動画内音声トラック、同期の基準音声、手動補正を選択できます。「文字起こしから始める」は空プロジェクトを作成してから本ページ後半に記載した共通文字起こしActionを呼び出します。既存プロジェクトを読み込む場合も、今回選択した素材と設定を引き継ぎます。専用の処理状態は持たず、完了・失敗・停止も共通経路で扱います。開始後に文字起こしを停止しても、作成済みの空プロジェクトは残ります。プロジェクトの読込・作成に失敗した場合は、開始画面に理由を表示します。
