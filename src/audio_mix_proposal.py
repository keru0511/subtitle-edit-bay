...
def build_audio_mix_context(
    ...
    *,
    ...
    if range_seconds is not None:
        if not isinstance(range_seconds, Sequence) or len(range_seconds) != 2:
            raise AudioMixProposalError("audio context range must contain start and end")
...
def apply_audio_mix_proposal(
    ...
    *,
    ...
    active_channels = active_audio_mix_channels(updated)
    if not allow_silence and not any(
        _bounded_level(channel.get("volume_percent", 100.0), maximum=MAX_VOLUME_PERCENT) > 0.0
