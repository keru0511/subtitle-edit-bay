...
class GuiTranscriptionContextStateTests(unittest.TestCase):
    ...
    def test_enabled_context_without_candidates_reuses_cached_metadata(self) -> None:
        state = gui_transcription_context_state_from_config(
            {
                "craig_pipeline": {
                    "transcription_context": {
                        "web_dictionary_enabled": True,
                        "web_dictionary_candidate_metadata": [
                            {"term": "Splatfest", "sources": [{"label": "title"}]},
                            {"term": "Ink", "sources": [{"label": "notes"}]},
                        ],
                    }
                }
            }
        ...
        self.assertEqual(
            state["web_dictionary_candidates"],
            ["Splatfest", "Ink"],
        )
        self.assertEqual(state["web_dictionary_candidate_metadata"][0]["term"], "Splatfest")
        self.assertEqual(state["web_dictionary_candidate_metadata"][1]["term"], "Ink")
    ...
    def test_config_context_is_rendered_as_gui_text_state(self) -> None:
        ...
        self.assertEqual(state["web_dictionary_candidate_metadata"], [])
