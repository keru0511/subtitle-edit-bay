...
class SubtitleEditorBackendTests(unittest.TestCase):
    def test_edit_undo_redo_and_autosave(self) -> None:
        ...
        with tempfile.TemporaryDirectory() as temp_dir:
            ...
            main_workspace = window.findChild(QQuickItem, "mainWorkspace")
            editor_page = window.findChild(QQuickItem, "editorPage")
            workflow_stepper = window.findChild(QQuickItem, "workflowStepper")
            self.assertTrue(main_workspace.isVisible())
            self.assertFalse(editor_page.isVisible())
            self.assertTrue(workflow_stepper.isVisible())

            settings_panel = window.findChild(QQuickItem, "advancedSettingsPanel")
            settings_toggle = window.findChild(QQuickItem, "settingsToggleButton")
            self.assertFalse(settings_panel.isVisible())
            center = settings_toggle.mapToScene(QPointF(settings_toggle.width() / 2, settings_toggle.height() / 2))
            QTest.mouseClick(window, Qt.MouseButton.LeftButton, pos=QPoint(round(center.x()), round(center.y())))
            app.processEvents()
            self.assertTrue(settings_panel.isVisible())
            workflow_actions = window.findChild(QQuickItem, "workflowActions")
            self.assertLessEqual(settings_panel.y() + settings_panel.height(), workflow_actions.y() + 1)
            self.assertLessEqual(workflow_actions.y() + workflow_actions.height(), workflow_actions.parentItem().height() + 1)
            ...
            self.assertFalse(main_workspace.isVisible())
            ...
            editor_back = window.findChild(QQuickItem, "editorBackButton")
            center = editor_back.mapToScene(QPointF(editor_back.width() / 2, editor_back.height() / 2))
            QTest.mouseClick(window, Qt.MouseButton.LeftButton, pos=QPoint(round(center.x()), round(center.y())))
            app.processEvents()
            self.assertTrue(main_workspace.isVisible())
            self.assertFalse(editor_page.isVisible())
